...
function showPosts( $current_page_num, $page_limit, $page_offset){
    ...
    $sql=  "SELECT magazine_posts.id,magazine_posts.mag_date,magazine_posts.city_id,cities.city_name from magazine_posts,cities where  cities.id=magazine_posts.city_id  order by magazine_posts.mag_date DESC LIMIT $page_limit OFFSET $page_offset  ";
    ...
    if(count($result) > 0){
        ...
        foreach($result as $row){
            $sqli=  "SELECT mag_image from magazine_post_image WHERE post_id=".$row['id']." order by id asc LIMIT 1 ";
            $resulti=$crud->getData($sqli);
            echo '<div class="col-md-4 col-12">
            <a href="'.FRONT_SITE_PATH.'avimat/'.$row['city_id'].'/'.$row['mag_date'].'" >
            <img class="list_img" src="'.SITE_MAGAZINE_IMAGE.$resulti[0]['mag_image'].'">
            <h2>'.$row['city_name'].'</h2>
            <p>'.$row['mag_date'].'</p>
            </a>
            </div>';
        ...
        $total_posts = count($crud->getData("SELECT id FROM `magazine_posts`"));
